...
export default function LeaderboardAnnouncement() {
  ...
  useEffect(() => {
    ...
    // Only show if we are before May 11th AND they haven't seen it yet
    if (!hasSeen && now < expirationDate) {
      // Slight 1-second delay so it pops up smoothly after the app loads
  ...
  return (
    <div className="fixed inset-0 z-[9999] pointer-events-auto flex flex-col items-center">
      
      {/* Blurred Dimmed Background (Clicking it skips the tour) */}
      <div 
        className="absolute inset-0 bg-zinc-900/60 backdrop-blur-[3px] transition-opacity duration-300" 
        onClick={handleDismiss} 
      />

      {/* STEP 1: Welcome Intro */}
      {step === 1 && (
        <div className="relative m-auto bg-white dark:bg-[#1c1c1c] rounded-[2rem] p-6 sm:p-8 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden mx-4">
      ...
      {step === 2 && (
        <div className="absolute bottom-24 right-4 sm:right-12 w-64 bg-white dark:bg-[#1c1c1c] rounded-3xl p-5 shadow-2xl animate-in slide-in-from-bottom-8 duration-300 border-2 border-amber-400">
          {/* Arrow pointing down */}
          <div className="absolute -bottom-3 right-8 w-6 h-6 bg-white dark:bg-[#1c1c1c] border-b-2 border-r-2 border-amber-400 transform rotate-45" />
          ...
          <button 
            onClick={handleNext}
            className="w-full py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-black uppercase tracking-widest text-[10px] shadow-sm active:scale-95 transition-transform"
          >
            {lang === 'sq' ? 'E Kuptova' : 'Got It'}
          </button>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="absolute top-[8.5rem] left-1/2 -translate-x-1/2 w-72 max-w-[90vw] bg-white dark:bg-[#1c1c1c] rounded-3xl p-5 shadow-2xl animate-in slide-in-from-top-8 duration-300 border-2 border-amber-400">
          {/* Arrow pointing up */}
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 font-medium mb-4">
            {lang === 'sq' 
              ...
              : "Once you are there, just select the 'The Last 30 Days' tab at the top to see who is dominating this month!"}
          </p>
          <button 
            onClick={handleNext}
